...
public class Prompt
{
	...
	private static InputStreamReader streamReader =
						new InputStreamReader(System.in); // reading in information strip down version
	private static BufferedReader bufReader = 
						new BufferedReader(streamReader); // created a buffer and fed it streamReader
	...
	public static int getInt (String ask)
	{
		...
		while (! found) {
			String str = getString(ask);
			try {
				val = Integer.parseInt(str); // try to see if it can find an integer
				found = true;
			}
	...
	public static double getDouble (String ask)
	{
		...
		while (! found) {
			String str = getString(ask);
			try {
				num =  Double.parseDouble(str); // try to see if it can find an integer
				found = true;
			}
			catch (NumberFormatException e) {
				found = false;
